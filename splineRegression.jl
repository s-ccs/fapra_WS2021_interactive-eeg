...
# ╔═╡ e5a56c78-5542-11ec-2e62-7f496442e8ae
begin
	using Unfold
	using CairoMakie
	using PlutoUI
	...
	using ColorSchemes # to get discrete colors
	using ColorTypes # to define alpha value
end
...
# ╔═╡ 9c75aa7a-5eeb-4094-9632-ae93afc25cb7
begin
	events = DataFrame(:continuousA => vcat(0,1,rand(MersenneTwister(1),n_samples-2))) # adding 0/1 to ensure same range for basis functions
	data = nonLinearFun.(events.continuousA).+ randn(MersenneTwister(1),n_samples).*σ
	data = reshape(data,1,1,n_samples) 
end;

# ╔═╡ 0406ad8a-ab5e-4d14-8383-5fcf4e7e9877
begin
	spl(t::Symbol, d::Int) = Unfold.uf_bsplineTerm(term(t), term(d))
	#spl_var = Base.Fix2(spl,n_splines)
	#f = @formula(0 ~ 1 + spl_var(continuousA))
	f = ConstantTerm(0)~ConstantTerm(1)+spl(:continuousA,n_splines)

	m_mul =fit(UnfoldModel, Dict(Any=>(f,[0.])), events, data)
	β = dropdims(coef(m_mul),dims=(1,2)) # get the coefficients as a vector

end;

# ╔═╡ fb70d1cb-dd23-441e-bcae-025230dda319
begin
	x = 0:0.005:1
	splBasis = Float64.(Unfold.genSplFunction(events.continuousA,n_splines)(x))
	splBasis = splBasis[:, Not(Int(ceil(end / 2)))] # remove middle spline
	fig1 = Figure()
	fig2 = Figure()
	fig3 = Figure()
	ax1 = Axis(fig1[1,1])
	...
	for k = 1:n_splines
		...
		(m,po) = findmax(abs.(singleBasis))
		posneg = sign(singleBasis[po]) 
		m=m*posneg# get the sign back
	...
	#t = Unfold.effects(Dict(:continuousA=>x,:latency=>0),m_mul) # via unfold pred
	#lines!(ax,t.continuousA,t.yhat,color="green",linewidth=10)
end;
...
git-tree-sha1 = "cd16dd9601e3ffb7174b0d74f3c0259175c6416b"
...
git-tree-sha1 = "890630e420539baf0f66194064244fa1950ef054"
...
git-tree-sha1 = "1eeaba1d77e3fdc69cb5df5970e34d4205147e8f"
