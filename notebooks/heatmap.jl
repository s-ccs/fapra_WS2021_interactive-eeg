...
md"What follows here is simply the code that allows us to plot the above interactive plot."

# ╔═╡ e239d793-af4e-470d-8b9a-50d6f00dc229
function deg2pix(d)
	px = d*1/0.036 # approximate
	return px
end;

# ╔═╡ 6289e7b0-527a-425b-9ca4-7491f86bafe0
md"### The Kernel"

# ╔═╡ c7008d98-61b5-4ced-bea9-f282e4f3ecb3
md"""
### Random Fields
To get a semi-realistic fixation density map, we generate some Gaussian Random Field. This is probably not the fastest way, but one of the first I thought of"
"""

# ╔═╡ 702c9bdf-80e7-401b-991b-dd04a0cb2338
begin
	# define a gaussianRandomField (slow)
...
begin
	...
	# stupid slow hack to sample according to 
	ranP = StatsBase.sample(Random.MersenneTwister(42),1:length(vecP),Weights(fun.(vecP)),npoints)
	indexP = [findfirst(vecP[k].==(p)) for k in ranP]
	data = Float64.([getindex.(indexP,1)*zoomFaktor getindex.(indexP,2)*zoomFaktor])
	data = data .+ zoomFaktor .* rand(size(data)...)
end;

# ╔═╡ 62ca9381-6ee2-4f87-8dfb-65774fe5fa73
 begin
	 kPx = deg2pix(kernelSizeVisAng*zoomFaktor);
	 kernelFunction = MvNormal([0,0],[2*kPx 0; 0 2*kPx]);
 	kernel = [pdf(kernelFunction,[i,j]) for i in -100:99, j in -100:99]
	heatmap(kernel)
 
 end

# ╔═╡ 6e2e265a-dd1e-4f6d-8259-f533967ad843
dataHist  = fit(Histogram, (data[:,1],data[:,2]),(0.51:1000*zoomFaktor,0.5:1000*zoomFaktor));

# ╔═╡ 442dfdfa-ca25-430c-9628-d40d9125c2b9
begin
f,a,h = heatmap(imfilter(dataHist.weights,kernel,Fill(0)))
if plotScatter
	scatter!(data[:,1],data[:,2],color="red")
end
	f
end
...
git-tree-sha1 = "dc84a6178fc8067322236b43536175a1d0dc2c5c"
